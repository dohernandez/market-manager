...
func (rw *WalletReload) Reload(w *wallet.Wallet) error {
	return transaction(rw.db, func(tx *sqlx.Tx) error {
		if err := rw.execDeleteTrade(tx, w); err != nil {
			return err
		}
...
func (rw *WalletReload) execDeleteTrade(tx *sqlx.Tx, w *wallet.Wallet) error {
	query := `DELETE FROM trade_operation WHERE trade_id IN (SELECT id FROM trade WHERE wallet_id = $1)`

	if _, err := tx.Exec(query, w.ID); err != nil {
		return err
	}

	query = `DELETE FROM trade WHERE wallet_id = $1`
